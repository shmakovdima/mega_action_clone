...
PhotoItem = React.createClass
  ...
  render: ->
    div {className: "col-sm-6 expert_item expert_item_win margin_top_60"},
...
PhotosGridComponent = React.createClass
  ...
  render: ->
    <div className="expert_body margin_top_20">
      <div className="row">
        <div>
          <div className="col-sm-6 expert_item expert_item_winnermain margin_top_60">
            <div className="expert_item_block">
              <div className="expert_item_img">
                <img className="img-responsive" src="http://change.megamall.ru/static/uploaded_images/document-1459527583783.jpg"/>
                <div className="expert_item_status">
                </div>
              </div>
              <div className="expert_item_footer">
                <div className="expert_item_title">
                  <div className="expert_item_title_img">
                  </div>
                  <div className="expert_item_title_text">Екатерина Климова
                  </div>
                </div>
                <div className="expert_item_text">Таня, пальто хорошее - оставляем. Рекомендую заменить штаны на джинсы-бойфренды или темные джеггинсы с дырами на коленях.
                </div>
              </div>
            </div>
...
PhotosFilterComponent = React.createClass
  ...
  getDatesText: (dates_id)->
    res = switch
      ...
      when dates_id is 4 then "30 марта — 9 апреля"
  render: ->
        ...
        <div className="col-md-4 expert_center_filter">
          <div className="expert_center_filter-block">
            ...
            <div className="expert_center_filter_hidden" ref="dates">
              ...
              <span onClick={@filter} data-value="4" defaultChecked="true">30 марта — 9 апреля</span>
            </div>
          </div>
        </div>
      </div>
...
window.PhotosComponent = React.createClass
  ...
  getButton: ()->
    ...
    text = switch
      when t <= 15*cp then ""
      when t < 15*(cp+1) then "Показать еще " + (t - 15*cp) + " мнений"
      when true then "Показать еще 15 мнений"
  ...
  render: ->
    div {}, 
      <h1 className="expert_header hidden-xs hidden-sm">Победители</h1>
      span {className: "expert_line expert_center_subtitle text-center-sm text-center-xs"}, "Загрузи свой лук, и стань одним из них!"
      span {className: "expert_center_description margin_top_20 text-left"}, "На этой неделе 15 карт «МЕГА подарок» по 3000 рублей достаются следующим участникам:"
      ...
      div {className: "expert_body expert_button margin_top_60 text_center"},
        @getButton()
...
loadPage = (num, filters, cb)->
  ...
  JCapi("/api/photo/winners", "GET", query)((res)->
