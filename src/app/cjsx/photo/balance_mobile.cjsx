...
BalanceMobileComponent = React.createClass
  getInitialState: ->
    {opened: false}
  open: (e)->
    e.preventDefault();
  ...
  render:->
    <div>
      <a href="#{}" onClick={@open} className="header_nav">Проверить баланс</a>
      { <RenderInBody><BalancePopupComponent close={@close} /></RenderInBody> if @state.opened }
    </div>
    

for el in document.getElementsByClassName('balance_mobile_place')
  React.render <BalanceMobileComponent />, el