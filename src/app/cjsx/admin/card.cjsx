...
AddRow = React.createClass
  ...
  handleAdd: ->
    JCapi("/api/preregister/transaction/find", "POST", {card_number: @refs.card_number.value, whos: @refs.whos.value, phone: @props.transaction.phone})(=>
      loadPage 1, @props.transaction.phone, @props.date_start, @props.date_end, @props.mega, @props.card_number
    )
  formatDate: (str)->
  ...
  render: ->
    <tr>
      <td className="text-center">
        {@props.transaction.lname}
      </td>
      <td className="text-center">
        {@props.transaction.fname}
      </td>
      <td className="text-center">
        {@props.transaction.phone}
      </td>
      <td className="text-center">
        {@formatDate(@props.transaction.created_at)}
      </td>
      <td className="text-center">
        <input onBlur={@handleEdit} ref="card_number" type="text" defaultValue="" className="text-center" />
      </td>    
      <td className="text-center">
        {@props.transaction.sum}
      </td>  
      <td className="text-center">
        <select defaultValue={@props.transaction.whos} onChange={@handleEdit} ref="whos">
...
TransactionsRow = React.createClass
  componentDidMount: ->
  ...
  handleRemove: ->
    swal {title: "Вы уверены?", text: "Удаление невозможно будет отменить", type: "warning", showCancelButton: true, confirmButtonText: "Да, удалить", cancelButtonText: "Нет", confirmButtonColor: "#4CAF50"}, (isConfirm)=>
      if isConfirm
        JCapi("/api/preregister/transaction/find/" + @props.transaction._id, "DELETE", {created_at: @props.transaction.created_at})(=>
          loadPage @props.currentPage, @props.phone, @props.date_start, @props.date_end, @props.mega, @props.card_number
        )
  handleEdit: ->
    setTimeout =>
      JCapi("/api/preregister/transaction/find/" + @props.transaction._id, "PUT", {card_number: @refs.card_number.value, whos: @refs.whos.value})((res)=>
        loadPage @props.currentPage, @props.phone, @props.date_start, @props.date_end, @props.mega, @props.card_number
      )(()=>
  ...
  render: ->
    <tr>
      ...
      <td className="text-center">
        <input onBlur={@handleEdit} ref="card_number" type="text" defaultValue={@props.transaction.card_number} className="text-center" />
      </td>    
      <td className="text-center">
        {@props.transaction.sum}
      </td>  
...
TransactionsTable = React.createClass
  getInitialState: ->
    {add: false}
  ...
  render: ->
    <div className="table-responsive overflow_auto">
      {<div className="btn btn-primary" onClick={@showAddButton}>Добавить</div> if !!@props.transactions and (@props.phone.length isnt 0) and (!@state.add) }
      <table className="table table-striped custab users_layout">
        <thead>
          <tr>
            ...
            <th className="text-center">Номер карты</th>
        ...
        <tbody>
          {<AddRow transaction={@props.transactions[0]} mega={@props.mega} /> if !!@props.transactions and (@props.phone.length isnt 0) and (@state.add)}
          {<TransactionsRow card_number={@props.card_number} mega={@props.mega} currentPage={@props.currentPage} transactions={@props.transactions} date_start={@props.date_start} date_end={@props.date_end} transaction={transaction} key={transaction._id} phone={@props.phone} /> for transaction in @props.transactions}
        </tbody>
      </table>
    </div>

TransactionsSearchComponent = React.createClass
  componentDidMount: ->
    $(@refs.phone).mask("+79999999999")
  onSubmit: (e)->
    e.preventDefault() if e isnt undefined
    loadPage 1, @refs.phone.value, @props.date_start, @props.date_end, @props.mega, @refs.card_number.value
  onRemove: ->
    @refs.phone.value = ""
    @onSubmit()
  render: ->
    <form className="form_horizontal border-no margin_top_20 overflow_hidden" onSubmit={@onSubmit}>
      <div className="form-group">
        ...
        <div className="input-group col-sm-10 col-sm-offset-1 margin_top_20">
          <input className="form-control phone" type="tel" id="phone_input" placeholder="Введите номер карты" ref="card_number" defaultValue={@props.card_number} />
          <span className="input-group-btn">
            <button className="btn btn-secondary" type="submit">
              <i className="fa fa-search"></i>
            </button>
...
TransactionsFilterComponent = React.createClass
  componentDidMount: ->
  ...
  onSubmit: (e)->
    e.preventDefault() if e isnt undefined
    loadPage 1, @props.phone, @refs.date_start.value, @refs.date_end.value, @refs.whos.value, @props.card_number
  handleEdit: ->
    setTimeout =>
      @onSubmit()
    , 0
...
TransactionsTablePagination = React.createClass
  goToPage: (e)->
...
TransactionsComponent = React.createClass
  render: ->
    <div>
      ...
      <TransactionsSearchComponent card_number={@props.card_number} mega={@props.mega} phone={@props.phone} date_start={@props.date_start} date_end={@props.date_end} />
      <TransactionsFilterComponent card_number={@props.card_number} mega={@props.mega} phone={@props.phone} date_start={@props.date_start} date_end={@props.date_end} />
      {<TransactionsTable card_number={@props.card_number} mega={@props.mega} currentPage={@props.currentPage} transactions={@props.transactions} phone={@props.phone} date_start={@props.date_start} date_end={@props.date_end} /> if @props.total isnt 0}
      {<div className="container text-center"><TransactionsTablePagination card_number={@props.card_number} mega={@props.mega} totalPages={@props.totalPages} currentPage={@props.currentPage} transactions={@props.transactions} phone={@props.phone} date_start={@props.date_start} date_end={@props.date_end} /></div> if @props.totalPages > 1}
    </div>

loadPage = (num, phone="", date_start="09.03.2016", date_end, mega=-1, card_number="")->
  date_end = moment().format("DD.MM.YYYY") if date_end is undefined
  JCapi("/api/preregister/transaction/find", "GET", {page: num, phone: phone, date_end: date_end, date_start: date_start, mega: mega, card_number: card_number})((res)->
    totalPages = Math.round(res.total/100)+1
    React.render <TransactionsComponent card_number={card_number} mega={mega} currentPage={num} totalPages={totalPages} date_start={date_start} date_end={date_end} transactions={res.transactions} phone={phone} total={res.total} />, document.getElementById('card_place')
  )
loadPage 1