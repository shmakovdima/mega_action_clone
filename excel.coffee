...
tool = 
  ...
  copy : (origin, target) ->
    if existsSync(origin)
      fs.mkdirSync(target) if not existsSync(target)
...
class Workbook
  ...
  save: (cb) =>
    console.log @fpath + '/' + @id
    target = @fpath + '/' + @id
    # 1 - build [Content_Types].xml
    fs.mkdir target, ()=>
      fs.writeFile target+'/[Content_Types].xml',@ct.toxml(),'utf8', (err)=>
        console.log "here am I", err
        # 2 - build docProps/app.xml
        fs.mkdir target+'/docProps', ()=>
          fs.writeFile target+'/docProps/app.xml',@da.toxml(),'utf8', =>
            # 3 - build xl/workbook.xml
            fs.mkdir target+'/xl', ()=>
              fs.writeFile target+'/xl/workbook.xml',@wb.toxml(),'utf8', =>
                # 4 - build xl/sharedStrings.xml
                fs.writeFile target+'/xl/sharedStrings.xml',@ss.toxml(),'utf8', =>
                  # 5 - build xl/_rels/workbook.xml.rels
                  fs.writeFile target+'/xl/_rels/workbook.xml.rels',@re.toxml(),'utf8', =>
                    # 6 - build xl/worksheets/sheet(1-N).xml
                    fs.writeFile target+'/xl/worksheets/sheet1.xml',@sheets[0].toxml(),'utf8', =>
                      # 7 - build xl/styles.xml
                      fs.writeFile target+'/xl/styles.xml',@st.toxml(),'utf8', =>
                        # 8 - compress temp folder to target file
                        zip = new EasyZip()
                        zip.zipFolder target+'/.', ()=>
                          zip.writeToFile(path.join(@fpath, @fname))
                          cb err, path.join(@fpath, @fname)
